      ...
      <div className="flex flex-col items-center gap-2.5 text-center">
        {block.showLogo && (
          <div className="size-20 overflow-hidden rounded-3xl border-4 bg-background shadow-lg" style={{ borderColor: `${ctx.theme.brand}55` }}>
            {ctx.business.logo_url ? <Photo src={ctx.business.logo_url} alt={`Logo ${ctx.business.name}`} width={80} height={80} sizes="80px" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-2xl font-bold" style={{ color: ctx.theme.brand }}>{ctx.business.name.charAt(0)}</div>}
          </div>
        )}
